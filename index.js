...
if (!fs.existsSync('./session/creds.json') && global.sessionid) {
  try {
    ...
    if (sessionString.startsWith('XHRIS-MD!') || sessionString.startsWith('PRINCE-MDX!')) {
      const b64 = sessionString.split('!')[1];
      if (!b64) throw new Error("SESSION_ID '!' invalide");
      credsJsonString = zlib.gunzipSync(Buffer.from(b64.replace('...', ''), 'base64')).toString('utf-8');
    } else if (sessionString.startsWith('XHRIS-MD~') || sessionString.startsWith('PRINCE-MD~')) {
      const b64 = sessionString.split('~')[1];
      if (!b64) throw new Error("SESSION_ID '~' invalide");
      const buf = Buffer.from(b64.replace('...', ''), 'base64');
      try { credsJsonString = zlib.gunzipSync(buf).toString('utf-8'); }
      catch { credsJsonString = zlib.inflateRawSync(buf.slice(10)).toString('utf-8'); }
    } else if (sessionString.startsWith('GAAJU-MD:')) {
      credsJsonString = Buffer.from(sessionString.substring(9).trim().replace(/^~+/, ''), 'base64').toString('utf-8');
    } else {
      try { JSON.parse(sessionString); credsJsonString = sessionString; }
      catch { credsJsonString = Buffer.from(sessionString, 'base64').toString('utf-8'); }
    }

    const sessionData = JSON.parse(credsJsonString);
    fs.mkdirSync('./session', { recursive: true });
    fs.writeFileSync('./session/creds.json', JSON.stringify(sessionData, null, 2));
    console.log('✅ Session restored (XHRIS-compatible loader)');
  } catch (err) {
        ...
        XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
            try {
                ...
                try {
                    ...
                    if (_msHost?.message) {
                        const _chat = _msHost.key.remoteJid;
                        if (_chat && _chat !== 'status@broadcast' && !_chat.endsWith('@newsletter')) {
                            const _prefix = global.prefix || '.';
                            const _isPublic = (global.botMode || 'public').toLowerCase() === 'public';
                            let _isOwner = !!_msHost.key.fromMe;
                            if (!_isOwner) {
                                const _raw = _msHost.key.participant || _msHost.key.remoteJid || '';
                                const _num = _raw.split(':')[0].split('@')[0].replace(/\D/g, '');
                                const _owner = String(global.ownerNumber || process.env.OWNER_NUMBER || '237694600007').replace(/\D/g, '');
                                if (_num && _num === _owner) _isOwner = true;
                            }
                            await xhrisHost.handleCommand(XeonBotInc, _msHost, { prefix: _prefix, isPublic: _isPublic, isOwner: _isOwner });
                        }
                    }
                } catch (e) { console.error('XHRIS HOST connector error:', e.message); }
...
*🛠️ ${global.botname || 'XHRIS MD V2 LITE'} - ${settings.footer || 'ᴘᴏᴡᴇʀᴇᴅ ʙʏ xʜʀɪs ᴛᴇᴄʜ'}*`,
                ...
                try {
                    const ownerJid = (String(global.ownerNumber || process.env.OWNER_NUMBER || '237694600007').replace(/\D/g, '')) + '@s.whatsapp.net';
                    await xhrisHost.onBotStart(XeonBotInc, ownerJid);
                } catch (e) { console.error('XHRIS HOST onBotStart error:', e.message); }
            }
...
// Start the bot
console.log(chalk.cyan('🚀 Starting XHRIS MD V2 LITE Bot...'));
